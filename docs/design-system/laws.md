# Aurora v2.0 — the fifteen hard laws

Source: `docs/luat-thiet-ke.md` §1, ratified 10/08. Translated, not rewritten —
every number, every threshold, every "only"/"never"/"single exception" carries
over exactly. See `docs/design-system/index.md` for the old-section →
new-file map.
...
1. **Color** only comes from the Pebble Vina brand palette plus the semantic
   tokens in `packages/tokens/globals.css`. `#22B573` is the one permitted
   derived semantic color (the brand palette has no green). Text placed
   **inside an already-tinted block** uses the `--on-tint-*` token group — do
   not invent a new tint.
2. **Pebble Blue `#133A8A` and Slate Gray `#5E6B80` are never text color** on a
   ...
   `#93A1B8`.
3. **Azure `#2E63E6` is background-only** — for AI, primary buttons, active
   state, countable on every screen. Azure text is always `#7FA3FF`.
...
5. **Corner radius**: card 6 · control 4 · tag 3 · `rounded-full` reserved for
   ...
   right corner).
6. **Type**: Archivo (display) · IBM Plex Sans (body) · Space Grotesk (hero
   numbers) · IBM Plex Mono (code, table numbers). Numbers are always
   `tabular-nums`. VN currency convention: comma for decimal, period for
   thousands.
...
11. **Icons**: Hugeicons Stroke Rounded, stroke 1.75, size 16 in buttons / 20
    ...
    icons, no emoji.
12. **Screen background** is exactly 4 layers: aurora glow (2 blurred blobs,
    90px) → 32px grid → 160px grid → SVG noise grain, `soft-light`,
    opacity .11. Do not add a fifth layer. Placed on the outermost frame of the
...
15. **No AI slop**: the AI Assistant icon is `orbit` (never `sparkles`, never
    ...
    ▲▼▬. No emoji, no decorative gradient outside the 4 background layers and
    the AI block background, no rounded-card-with-left-border-accent.
...
- **Law 12** — background is exactly 4 layers, no fifth layer
- **Law 13** — contrast ≥ 4.5:1 on both `.glass-a` and `.glass-b`; tablet
  buttons ≥ 48px
