...
| 3   | **Poll 4s** (`PV_QUEUE_POLL_SECONDS`) + `batchSize: 2` bật burst   | Vẫn là một khoản tiền chứ không chỉ là độ trễ — nhưng nay chỉ trả MỘT LẦN, cho khoảng chờ trước lá đầu. Trần tốc độ từng nằm ở `batchSize: 1`: pg-boss bỏ qua `burstWhenBatchFull` ở batch 1, nên worker gửi một thư rồi ngủ trọn một nhịp — 2 thư/12s trên một cửa cho 4 thư/giây. Nhịp gửi thật vẫn là `PV_EMAIL_RATE_PER_SECOND` |
