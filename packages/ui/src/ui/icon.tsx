...
export type IconProps = {
  icon: IconGlyph
  /** 16 trong nút · 20 trong nav · 24–26 cho ô quét và empty state · 64 chỉ
   *  dành cho watermark trang trí chìm trong nền StatCard
  ...
  size?: 14 | 16 | 17 | 18 | 20 | 24 | 26 | 64
  /** 1.75 mặc định; 1.9 chỉ cho icon nằm trên nền azure đặc (nút quét, khối AI) */
  strokeWidth?: 1.75 | 1.9
  className?: string
}

export function Icon({ icon, size = 16, strokeWidth = 1.75, className }: IconProps) {
  return (
    <HugeiconsIcon
      icon={icon}
      size={size}
