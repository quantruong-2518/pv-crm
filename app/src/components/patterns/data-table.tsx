...
export function DataTable({
  ...
  return (
    <div className={className} role="table">
      <div
        role="row"
        className="grid border-b border-b-white/6 pb-2 text-[11px] text-muted-foreground"
        style={{ gridTemplateColumns: template }}
      >
        {columns.map((col) => (
          <span key={col.header} role="columnheader" className={cn(col.align === 'right' && 'text-right')}>
            {col.header}
          </span>
        ))}
      </div>
      ...
      {rows.map((row, i) => (
        <div
          ...
          className={cn(
            ...
            i < rows.length - 1 && 'border-b border-b-white/6',
        ...
        >
          {row.cells.map((cell, c) => (
            <span
              key={c}
              role="cell"
              className={cn(columns[c]?.align === 'right' && 'text-right')}
            >
              {cell}
            </span>
          ))}
