    ...
    >
      {ITEMS.map((item) => {
        ...
        return (
          ...
          >
            <Icon icon={item.icon} size={20} className={active_ ? 'text-accent-foreground' : undefined} />
            <span className="text-[10px] font-medium">{item.label}</span>
            {item.key === 'approvals' && Boolean(approvalsCount) && (
              <span className="absolute top-0 right-2 rounded-sm bg-destructive px-[5px] py-px text-[9.5px] font-semibold text-primary-foreground">
                {approvalsCount}
              </span>
            )}
          </button>
